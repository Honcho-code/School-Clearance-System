  ...
  return (
    <div className="mb-4">
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-semibold uppercase tracking-widest text-[#4B5680] dark:text-[#8B97B8]">
          {label}
        </p>
        {isComplete ? (
...
export default function Apply() {
  ...
  const handleSubmit = async () => {
    const missingSchoolFees = LEVEL_KEYS.filter(k => (files.schoolFees[k] || []).length === 0)
    const missingMedical    = LEVEL_KEYS.filter(k => (files.medical[k]    || []).length === 0)
    const missingLibrary    = files.library.length === 0

    if (missingSchoolFees.length || missingMedical.length || missingLibrary) {
      const parts = []

      if (missingSchoolFees.length) {
        const labels = missingSchoolFees.map(k => k.replace('-', ' — ') + ' Sem').join(', ')
        parts.push(`School Fees (${labels})`)
      }
      if (missingMedical.length) {
        const labels = missingMedical.map(k => k.replace('-', ' — ') + ' Sem').join(', ')
        parts.push(`Medical (${labels})`)
      }
      if (missingLibrary) {
        parts.push('Library Receipt')
      }

      toast.error(
        `Please upload all required receipts before submitting. Missing: ${parts.join(' • ')}`,
        { duration: 6000 }
      )

      // Jump the user to the first incomplete step
      if (missingSchoolFees.length) setStep(1)
      else if (missingMedical.length) setStep(2)
      else setStep(3)
