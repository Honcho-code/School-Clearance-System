...
export default function MedicalReview() {
  ...
  const load = () => {
    getAllApps().then(all => {
      const queue = appId
        ...
        : all.filter(a => a.stages.medical?.status === 'reviewing')
  ...
  const submit = async (decision) => {
    ...
    setSaving(true)
    try {
      await stagesAPI.review(active, { stage: 'medical', decision, remark })
  ...
  return (
    <div className="max-w-[800px] mx-auto">
      <PageHeader title="Review Medical Receipts" subtitle={`${apps.length} application${apps.length !== 1 ? 's' : ''} in queue`}/>
      {busy ? <p className="text-sm text-[#8A94B0] text-center py-12">Loading...</p>
        : apps.length === 0
          ? <div className="card"><Empty icon={Stethoscope} title="Queue empty" body="No medical receipts pending review."/></div>
          : apps.map(app => {
            const stage = app.stages.medical
            return (
              <div key={app.id} className="card p-5 mb-4">
                ...
                {/* ── Medical receipts viewer ── */}
                <ReceiptViewer receipts={app.receipts?.medical || []} title="Medical Receipts — click to preview"/>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === 'approve' ? 'Approve Medical Receipts' : 'Query Submission'}>
